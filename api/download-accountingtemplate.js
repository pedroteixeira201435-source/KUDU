const { PRODUCT, verifyDownloadToken, resolveOrder, getProductBuffer } = require('./_lib');
...
module.exports = async function handler(req, res) {
  ...
  try {
    const { fulfilled } = await resolveOrder(verified.orderId);
    if (!fulfilled) return res.status(403).json({ error: 'Payment is not confirmed' });
